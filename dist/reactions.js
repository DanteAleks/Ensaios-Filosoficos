(() => {
 ...
 const key='peregrini:reaction:'+work,identityKey='peregrini:reaction-voter:v1';let voter='',selected='none',busy=false,available=false;
 function render(){buttons.forEach(b=>{b.disabled=busy||!available;b.setAttribute('aria-pressed',String(selected===b.dataset.vote));});}
 ...
 async function init(){try{voter=localStorage.getItem(identityKey)||'';if(!/^[a-f0-9]{32}$/.test(voter)){voter=Array.from(crypto.getRandomValues(new Uint8Array(16)),b=>b.toString(16).padStart(2,'0')).join('');localStorage.setItem(identityKey,voter);}const stored=localStorage.getItem(key);if(['like','dislike'].includes(stored))selected=stored;}catch{message.textContent='Permita o armazenamento neste navegador para registrar sua avaliação.';return;}
  try{await request('/counts?work='+encodeURIComponent(work));available=true;message.textContent='';}catch{available=false;message.textContent='Avaliações indisponíveis no momento.';}render();
 }

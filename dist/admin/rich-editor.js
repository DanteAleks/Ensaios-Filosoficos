(()=>{
  ...
  window.mountDocumentEditor=(host,v,changed)=>{
    ...
    function insertFormula(display){
      ...
      document.execCommand('insertText',false,display?'\\['+value+'\\]':'\\('+value+'\\)');sync();remember();
    }
    ...
    for(const [display,label,attribute] of [[false,'LaTeX em linha','data-latex-inline'],[true,'LaTeX em destaque','data-latex-display']]){
      const button=document.createElement('button');button.type='button';button.textContent=label;button.setAttribute(attribute,'');button.onmousedown=event=>event.preventDefault();button.onclick=()=>insertFormula(display);toolbar.append(button);
    }

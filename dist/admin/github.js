(function(root){
  ...
  function createClient(fetcher=fetch){
    ...
    async function read(){const file=await request(`${BASE}/contents/content/obras.json?ref=${encodeURIComponent(branch)}`);if(file.encoding==='base64'&&file.content)return {sha:file.sha,text:decode(file.content)};if(!/^[a-f0-9]{40,64}$/.test(file.sha))throw new Error('O catálogo retornado não pôde ser lido.');const blob=await request(`${BASE}/git/blobs/${file.sha}`);if(blob.encoding!=='base64'||!blob.content)throw new Error('Não foi possível ler o catálogo completo.');return {sha:file.sha,text:decode(blob.content)};}
